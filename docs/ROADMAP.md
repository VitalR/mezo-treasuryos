...
Tigris `mcbBTC/BTC` is the cleanest current BTC-correlated yield candidate because it preserves BTC-like exposure better than BTC/stable LP. Manual Mezo testnet transaction inspection confirms the UI path uses ERC20-style BTC at the BTCCaller/precompile address, so the old "native BTC mechanics unknown" blocker is reduced. V1 now supports proposal-only BTC sleeve planning from idle BTC reserve using pool reserves, a BTC -> mcbBTC quote, min-out/min-LP calculations, and BTCReservePolicy guardrails. Execution still belongs in V1.5 until TreasuryOS has a guarded BTC handler with swap min-out, LP min-liquidity, receipt/staked-LP accounting, price-impact/slippage policy, and elevated multisig approval flow. Tigris `MUSD/BTC` and BTC/MUSD concentrated liquidity should be treated as directional BTC/stable strategies, not as the default BTC treasury yield story.

V1 BTC sleeve scope is deliberately a calculator plus policy gate plus advisor memo. It should show MUSD Savings execution, optional MUSD/mUSDC execution, and a BTC sleeve planner that blocks mcbBTC/BTC when price impact, reserve floors, or approval level fails policy. The guarded `BTCReserveRouter` belongs in V1 as a scoped design boundary only, not as live principal movement. V1.5 should add controlled broadcast validation for BTC -> mcbBTC swap, add/remove liquidity, LP stake/unstake, reward claim, receipt accounting, and unwind before enabling guarded execution.
